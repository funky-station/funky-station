...
public sealed partial class MalfVoiceModulatorWindow : DefaultWindow
{
    ...
    public MalfVoiceModulatorWindow(Font? malfFont)
    {
        ...
        // Apply Malf font to key controls explicitly.
        if (malfFont != null)
        {
            if (ConfirmButton.Label != null)
                ConfirmButton.Label.FontOverride = malfFont;

            HeaderLabel.FontOverride = malfFont;

            // Add a local stylesheet so any other labels and text inputs in this window use the Malf font too.
            // Also make placeholder text transparent
            RootBackdrop.Stylesheet = new Stylesheet(new[]
            {
                ...
                new StyleRule(new SelectorElement(typeof(LineEdit), null, null, null),
                    new[] { new StyleProperty("font", malfFont) }),
                new StyleRule(new SelectorElement(typeof(LineEdit), null, LineEdit.StylePseudoClassPlaceholder, null),
                    new[] { new StyleProperty("font-color", Color.Transparent) })
            });
        }

        ConfirmButton.StyleBoxOverride = buttonStyle;
        HeaderLabel.Modulate = MalfUiTheme.Accent;
        NameEdit.Modulate = MalfUiTheme.Accent;
