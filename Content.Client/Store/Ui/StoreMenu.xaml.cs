...
using Robust.Client.UserInterface; // Stylesheet, StyleRule, SelectorElement, StyleProperty
...
public sealed partial class StoreMenu : DefaultWindow
{
    ...
    public void ApplyMalfTheme()
    {
        ...
        // Apply Malf font to the window contents via a local stylesheet
        RootBackdrop.Stylesheet = new Stylesheet(new[]
        {
            new StyleRule(new SelectorElement(typeof(RichTextLabel), null, null, null),
                new[] { new StyleProperty("font", _malfFont!) }),
            new StyleRule(new SelectorElement(typeof(Label), null, null, null),
                new[] { new StyleProperty("font", _malfFont!) })
        });

        // LineEdit may not support FontOverride; only tint.
        SearchBar.Modulate = MalfGreen;

        // Apply font to button labels where available, and tint.
        if (WithdrawButton.Label != null)
        {
            WithdrawButton.Label.FontOverride = _malfFont;
            WithdrawButton.Label.Modulate = MalfGreen;
        }
        else
        {
            WithdrawButton.Modulate = MalfGreen;
        }

        if (RefundButton.Label != null)
        {
            RefundButton.Label.FontOverride = _malfFont;
            RefundButton.Label.Modulate = MalfGreen;
        }
        else
        {
            RefundButton.Modulate = MalfGreen;
        }
    ...
    public void PopulateStoreCategoryButtons(HashSet<ListingData> listings)
    {
        ...
        if (_malfThemeApplied)
        {
            ...
            foreach (var id in malfCats)
            {
                ...
                if (_malfFont != null)
                {
                    if (catButton.Label != null)
                    {
                        catButton.Label.FontOverride = _malfFont;
                        catButton.Label.Modulate = MalfGreen;
                    }
                    else
                    {
                        catButton.Modulate = MalfGreen;
                    }
