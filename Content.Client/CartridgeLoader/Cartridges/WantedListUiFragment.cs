...
public sealed partial class WantedListUiFragment : BoxContainer
{
    ...
    private void OnItemSelected(BaseButton.ButtonEventArgs args, ListData data)
    {
        ...
        PersonState.SetMessage(GetLoc(
            ...
            ("status", _prototypeManager.Index(record.Status).LocalizedCartridgeLabel)
        ));
    ...
    private void GenerateItem(ListData data, ListContainerButton button)
    {
        ...
        var criminalStatus = _prototypeManager.Index<SecurityStatusPrototype>(record.Status);
        if (!criminalStatus.IsNone())
        {
            if (criminalStatus.SecurityIcon != null)
            {
                rect.Texture = _spriteSystem.Frame0(_prototypeManager.Index<SecurityIconPrototype>(criminalStatus.SecurityIcon).Icon);
            }
        }
