...
#pragma warning restore CS8774 // Member must have a non-null value when exiting.
    ...
    protected async Task AttemptShoot(NetCoordinates? target = null, bool assert = true)
    {
        ...
        {
            SGun.AttemptShoot(SPlayer, gunUid, gunComp!, actualTarget);
            if (assert)
                Assert.That(true,"Gun failed to shoot.");
        });
        await RunTicks(1);
    ...
    protected async Task AttemptShoot(NetEntity? target = null, bool assert = true)
    {
        ...
        await Server.WaitAssertion(() =>
        {
            SGun.AttemptShoot(SPlayer, gunUid, gunComp!, Position(actualTarget!.Value), ToServer(actualTarget!.Value));
            if (assert)
                Assert.That(true, "Gun failed to shoot.");
        });
        await RunTicks(1);
