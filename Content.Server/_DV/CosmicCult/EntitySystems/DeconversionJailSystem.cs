...
public sealed partial class DeconversionJailSystem : SharedDeconversionJailSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        {
            if (comp.OublietteState == OublietteStates.Active && Timing.CurTime > comp.EmoteTime)
            {
                comp.EmoteTime = Timing.CurTime + Random.Next(comp.EmoteMinTime, comp.EmoteMaxTime);
                _chat.TryEmoteWithChat(comp.Victim, "Scream", ChatTransmitRange.Normal, false, null, true, true);
                PopUp.PopupEntity(Loc.GetString("cosmic-oubliette-random-horror", ("COUNT", Random.Next(1, 7))), comp.Victim, comp.Victim, PopupType.MediumCaution);
            }
        }
    }
